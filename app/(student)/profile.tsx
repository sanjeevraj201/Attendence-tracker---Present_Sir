...
import { LogOut, Moon, Sun, Monitor, Camera, Trash2, Wifi, ShieldCheck, RefreshCw } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  const { isWifiVerificationEnabled, collegeBssid, collegeWifiName, setIsWifiVerificationEnabled, setCollegeBssid } = useSettingsStore();
  ...
  const handleFetchCurrentWifi = async () => {
    try {
      ...
      if (status !== 'granted') {
        alert('Permission to access location is required to read Wi-Fi information.');
      ...
      if (netInfo.type === 'wifi' && netInfo.details) {
        // netInfo.details on wifi contains bssid and ssid
        const bssid = (netInfo.details as any).bssid;
        const ssid = (netInfo.details as any).ssid;
        
        if (bssid) {
          setCollegeBssid(bssid, ssid);
          setIsWifiVerificationEnabled(true);
          alert(`Successfully set target Wi-Fi to ${ssid || bssid}`);
        } else {
          alert('Could not retrieve Wi-Fi BSSID. Are you connected to Wi-Fi?');
        }
      } else {
        alert('You do not appear to be connected to a Wi-Fi network.');
      }
    } catch (error) {
      alert('Failed to get Wi-Fi information.');
      console.error(error);
    } finally {
      setIsFetchingWifi(false);
    }
  ...
  return (
    <SafeAreaView className="flex-1 bg-offWhite dark:bg-oledBlack">
        ...
        <Text className="text-[13px] font-bold text-gray-400 dark:text-gray-500 mb-3 ml-2 uppercase font-sans tracking-widest">Verification Settings</Text>
        <View className="bg-white dark:bg-cardDark rounded-[24px] p-5 border border-black/5 dark:border-white/5 mb-8 shadow-sm dark:shadow-none">
          <View className="flex-row items-center justify-between mb-4 ml-1">
            <View className="flex-row items-center">
              <ShieldCheck color="#FF5E00" size={22} className="mr-3" />
              <Text className="text-[17px] font-black font-sans text-gray-900 dark:text-white tracking-tight">Wi-Fi Attendance</Text>
            </View>
            <TouchableOpacity 
              onPress={() => setIsWifiVerificationEnabled(!isWifiVerificationEnabled)}
              className={`w-[52px] h-[32px] rounded-full px-1 justify-center transition-colors ${isWifiVerificationEnabled ? 'bg-primaryOrange' : 'bg-gray-200 dark:bg-oledBlack border border-black/5 dark:border-white/5'}`}
            >
              <View className={`w-[24px] h-[24px] rounded-full shadow-sm ${isWifiVerificationEnabled ? 'self-end bg-white' : 'self-start bg-white dark:bg-gray-500'}`} />
            </TouchableOpacity>
          </View>
          
          <Text className="text-[14px] text-gray-500 dark:text-gray-400 font-sans ml-1 mb-6 leading-5">
            Require connection to a specific Wi-Fi network to mark attendance as Present.
          </Text>

          <View className="bg-gray-50 dark:bg-oledBlack p-4 rounded-[20px] border border-black/5 dark:border-white/5 mb-5">
            <Text className="text-[11px] font-black text-gray-400 dark:text-gray-500 mb-2 uppercase font-sans tracking-widest">Target Network</Text>
            {collegeBssid ? (
              <View>
                <Text className="text-[17px] font-bold text-gray-900 dark:text-white font-sans tracking-tight">{collegeWifiName || 'Unknown Network'}</Text>
                <Text className="text-[13px] font-black text-gray-400 font-mono tracking-widest mt-1 uppercase">{collegeBssid}</Text>
              </View>
            ) : (
              <Text className="text-[14px] italic text-gray-400 dark:text-gray-500 font-sans">No network configured.</Text>
            )}
          </View>

          <TouchableOpacity 
            className="flex-row items-center justify-center p-4 bg-primaryOrange/10 dark:bg-primaryOrange/5 rounded-[16px] border border-primaryOrange/20"
            onPress={handleFetchCurrentWifi}
            disabled={isFetchingWifi}
          >
            {isFetchingWifi ? (
              <RefreshCw color="#FF5E00" size={20} />
            ) : (
              <Wifi color="#FF5E00" size={20} />
            )}
            <Text className="ml-3 font-black font-sans text-primaryOrange text-[15px]">
              {isFetchingWifi ? 'Scanning...' : 'Set to Current Wi-Fi'}
            </Text>
          </TouchableOpacity>
        </View>
